    ...
    .then(async () => {
        ...
        if (await StatusSchema.findOne({m_code:session.m_code,date:moment().format("YYYY-MM-DD")})){
          var send_time = await StatusSchema.findOne({m_code:session.m_code,date:moment().format("YYYY-MM-DD")});
            ...
            await StatusSchema.findOneAndUpdate({m_code:session.m_code,date:moment().format("YYYY-MM-DD")},{time_end:""});
    ...
    .then(async () => {
      var date = moment().format("YYYY-MM-DD");
       if (choice != "POSTE"){
        ...
        await UserSchema.findOneAndUpdate({m_code:session.m_code},{act_loc:choice});
       }
    leave_checking = true;
       res.redirect("/exit_u");
    });
  ...
  if (session.occupation_a == "Admin"){
    ...
    .then(async () => {
      if (session.filtrage == "" && data_desired.datatowrite){
        ...
        var timesheets =  await StatusSchema.find({time_end:{ $ne: "" }}).limit(200);
...
routeExp.route("/fiche").get( function (req, res) {
  ...
  .then(async () => {
    ...
    res.render("paie.html",{opt:opt.paie_generated,list:opt.list_paie});
  })
